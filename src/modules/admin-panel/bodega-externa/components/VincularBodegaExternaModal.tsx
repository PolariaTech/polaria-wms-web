...
import {
  ...
  type BodegaExternaDisponibleRow,
} from "../services/bodegas-externas-admin.service";

interface VincularBodegaExternaModalProps {
  open: boolean;
  onClose: () => void;
  onLinked: () => void;
}

export function VincularBodegaExternaModal({
  open,
  onClose,
  onLinked,
}: VincularBodegaExternaModalProps) {
  const { codigoCuenta, codigoEmpresa } = useCompany();
  const [options, setOptions] = useState<BodegaExternaDisponibleRow[]>([]);
  ...
  useEffect(() => {
    ...
    void listBodegasExternasDisponiblesAdmin({ codigoCuenta, codigoEmpresa })
      .then(setOptions)
      .catch(() => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await vincularBodegaExternaAdmin({
        codigoCuenta,
        codigoEmpresa,
        idBodega: selectedId,
      });
